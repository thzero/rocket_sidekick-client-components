...
export function useChecklistComponent(props, context, options) {
	const {
		...
		preCompleteOk: async (correlationId) => {
			...
			const response = await serviceStore.dispatcher.saveChecklist(
				correlationId,
	...
	const clickRemoveLocation = async (item) => {
		dialogDeleteConfirmationParams.value = {
			id: detailItemLocationId.value,
			type: "location",
		};
	...
	const clickRemoveRocketSetup = async (item) => {
		dialogDeleteConfirmationParams.value = {
			id: detailItemRocketSetupId.value,
			type: "rocketSetup",
		};
		dialogDeleteConfirmationManager.value.open();
	};
	...
	const clickViewRocketSetup = async () => {
		...
		LibraryClientUtility.$navRouter.push(
			"/user/rocketsetups/" + item.rocketSetupId
		);
	};
	...
	const dialogDeleteConfirmationOk = async (correlationId) => {
		try {
			...
			if (dialogDeleteConfirmationParams.value.type === "rocket") {
				await removeRocket(correlationId);
				return;
			}
			if (dialogDeleteConfirmationParams.value.type === "rocketSetup") {
	...
	const updateOrder = async (payload, addedIndex, removedIndex) => {
		...
		if (item.steps) {
			...
			if (addedIndex !== null) item.steps.splice(addedIndex, 0, itemToAdd);
		}
	...
	const updateStatus = async (correlationId, id, status, launch) => {
		...
		if (!item) throw Error("Invalid item in updateStatus...");
		...
		if (launch) {
			...
			detailItemData.value.statusId =
				AppCommonConstants.Checklists.ChecklistStatus.completed;
		}
