...
export function useRocketsBaseComponent(props, context, options) {
	...
	const rockets = ref([]);
	const title = ref(
		(props.type === AppCommonConstants.Rocketry.RocketTypes.Yours ? LibraryClientUtility.$trans.t('titles.rockets.yours') + ' ' : '') + LibraryClientUtility.$trans.t('titles.rockets.title')
	);
