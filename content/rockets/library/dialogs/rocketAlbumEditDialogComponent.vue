...
export function useRocketAlbumEditDialogComponent(props, context, options) {
	const {
		correlationId,
		error,
		hasFailed,
	...
	const displayName = computed(() => {
		return LibraryClientUtility.$trans.t('forms.content.rockets.name') + ' ' + LibraryClientUtility.$trans.t('forms.content.rockets.albums.title');
	});
	...
	return {
		...
		resetAdditional,
		setAdditional,
		scope: 'RocketAlbumEditDialog',
		validation: useVuelidate({ $scope: 'RocketAlbumEditDialog' })
	};
};
</script>
