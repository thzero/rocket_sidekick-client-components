...
export function useRocketVideoEditDialogComponent(props, context, options) {
	const {
		correlationId,
		error,
		hasFailed,
	...
	const displayName = computed(() => {
		return LibraryClientUtility.$trans.t('forms.content.rockets.name') + ' ' + LibraryClientUtility.$trans.t('forms.content.rockets.videos.title');
	});
	...
	return {
		...
		resetAdditional,
		setAdditional,
		scope: 'RocketVideoEditDialog',
		validation: useVuelidate({ $scope: 'RocketVideoEditDialog' })
	};
};
</script>
