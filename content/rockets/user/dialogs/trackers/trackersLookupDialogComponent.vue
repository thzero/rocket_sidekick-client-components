...
export function useTrackersRocketLookupDialogComponent(props, context, options) {
	const {
		correlationId,
		error,
		hasFailed,
	...
	const dialogTrackersLookup = ref(null);
	...
	const clickTrackersSearch = async () => {
		await dialogTrackersLookup.value.submit(correlationId());
	};
	const clickTrackersSearchClear = async () => {
		await dialogTrackersLookup.value.reset(correlationId(), null, true);
	};
	const clickTrackersSelect = async (item) => {
	...
	const dialogResetOk = async () => {
		...
		await serviceStore.dispatcher.requestPartsTrackersSearchReset(correlationIdI);
	...
	const preCompleteOk = async (correlationId) => {
		...
		const temp = await serviceStore.dispatcher.requestPartsTrackersSearchResults(correlationId, request);
	...
	onMounted(async () => {
		...
		let temp2 = response.results.filter(l => l.types.find(j => 
			(j === AppCommonConstants.Rocketry.ManufacturerTypes.tracker)
		));
	...
	return {
		...
		preCompleteOk,
		resetAdditional,
		scope: 'TrackersLookupDialog',
		validation: useVuelidate({ $scope: 'TrackersLookupDialog' })
	};
};
</script>
